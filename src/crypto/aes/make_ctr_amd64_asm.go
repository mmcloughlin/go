...
func (a *AES) Generate() error {
	...
	for n := a.n; n > 0; n /= a.d {
		...
		a.inst("JB", "enc"+strconv.Itoa(n/a.d))
		a.Encrypt(n, name)
		a.Xor(n)
		a.inst("JMP", name)
	}
...
// Xor n blocks into src and writes to dst.
func (a *AES) Xor(n int) {
	a.section("XOR with src.")
	for i := 0; i < n; i++ {
		a.inst("MOVOU", "%d(SRC_PTR), TX", 16*i)
		a.inst("PXOR", "TX, B%d", i)
		a.inst("MOVOU", "B%d, %d(DST_PTR)", i, 16*i)
	}

	a.inst("ADDQ", "$%d, SRC_PTR", 16*n)
	a.inst("SUBQ", "$%d, SRC_LEN", 16*n)
	a.inst("ADDQ", "$%d, DST_PTR", 16*n)
}

func (a *AES) header() {
	fmt.Fprintln(a.w, "// Copyright 2018 The Go Authors. All rights reserved.")
	fmt.Fprintln(a.w, "// Use of this source code is governed by a BSD-style")
	fmt.Fprintln(a.w, "// license that can be found in the LICENSE file.")
...
func main() {
	...
	if err := ioutil.WriteFile("ctr_amd64.s", buf.Bytes(), 0666); err != nil {
		log.Fatal(err)
	}
}
